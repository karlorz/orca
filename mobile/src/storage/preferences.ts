...
export async function savePushNotificationsEnabled(
  enabled: boolean
): Promise<void> {
...
export async function saveTerminalAutocompleteEnabled(
  enabled: boolean
): Promise<void> {
...
function terminalLiveInputDisabledKey(
  hostId: string,
  worktreeId: string
): string {
...
export async function loadDisabledTerminalLiveInputHandles(
  ...
  const preference = await readDisabledTerminalLiveInputHandlesPreference(
    hostId,
    worktreeId
  );
...
export function clampHostSidebarWidth(width: number): number {
  ...
  return Math.min(
    ...
    Math.max(HOST_SIDEBAR_MIN_WIDTH, Math.round(width))
  );
}
...
export async function saveHostSidebarWidth(width: number): Promise<void> {
  await AsyncStorage.setItem(
    SIDEBAR_WIDTH_KEY,
    String(clampHostSidebarWidth(width))
  );
}
...
export function clampHostDockWidth(width: number): number {
  ...
  return Math.min(
    HOST_DOCK_MAX_WIDTH,
    Math.max(HOST_DOCK_MIN_WIDTH, Math.round(width))
  );
}
...
export const DEFAULT_TERMINAL_LINK_OPEN_MODE: MobileTerminalLinkOpenMode =
  "orca-browser";
...
export async function loadTerminalLinkOpenMode(): Promise<MobileTerminalLinkOpenMode> {
  try {
    ...
    return raw === "phone-browser" || raw === "orca-browser"
      ? raw
...
export async function saveTerminalLinkOpenMode(
...
export async function savePinnedIds(
  hostId: string,
  ids: Set<string>
): Promise<void> {
...
export async function loadVisibleUsageProviders(): Promise<
  Set<UsageProviderKey>
> {
  try {
...
export async function saveVisibleUsageProviders(
  ids: ReadonlySet<UsageProviderKey>
): Promise<void> {
...
let visibleUsageWrite: Promise<Set<UsageProviderKey>> = Promise.resolve(
  new Set()
);
...
export async function loadVisibleUsageProvidersSettled(): Promise<
  Set<UsageProviderKey>
> {
  while (true) {
