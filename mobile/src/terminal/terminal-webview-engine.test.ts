...
describe('terminal WebView bundled engine', () => {
  ...
  it('retries the default native notify seam until ReactNativeWebView is injected', () => {
    const seamSource = documentModuleSource('document-host-seams')
    expect(seamSource).toContain('const payload = JSON.stringify(message)')
    expect(seamSource).toContain('setTimeout(send, 50)')
    expect(seamSource).toContain('attempts < 100')
    expect(documentModuleSource('host-notify')).toContain('scope.postToHost(msg)')
  })
})
