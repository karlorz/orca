  ...
  it('is what the modules emit, modulo the eight counted classes', async () => {
    ...
    expect(compareTerminalDocumentScripts(preFlipScript, candidate, 'scope')).toEqual({
      ...
      normalisations: {
        ...
        // plus notify's payload/attempts after the speech retry-until-bridge change.
        rebindings: 375,
        // `curly`, measured over the whole script before any of this started, plus
        // the retry loop's `if (attempts < 100)` body.
        bracedBodies: 280,
