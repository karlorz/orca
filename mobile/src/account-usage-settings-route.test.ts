...
describe("account usage settings route", () => {
  beforeEach(() => {
    ...
    dependencies.setUsageProviderVisible
      .mockReset()
      .mockResolvedValue(new Set(["claude", "codex"]));
  ...
  it("rolls an optimistic toggle back when persistence fails", async () => {
    dependencies.setUsageProviderVisible.mockRejectedValueOnce(
    ...
    const antigravity = () =>
      renderer.root
        ...
        .find(
          (item) => item.props.accessibilityLabel === "Show Antigravity usage"
        );
    ...
    expect(dependencies.setUsageProviderVisible).toHaveBeenCalledWith(
      "antigravity",
      true
    );
    expect(dependencies.loadVisibleUsageProvidersSettled).toHaveBeenCalledTimes(
      2
    );
    expect(antigravity()?.props.value).toBe(false);
