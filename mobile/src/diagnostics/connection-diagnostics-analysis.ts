...
function selectDiagnosticFailure(
  ...
  for (let index = entries.length - 1; index >= 0; index--) {
    if (isSessionBoundary(entries[index])) {
  ...
  let boundaryIndex = -1
  for (let index = sinceSession.length - 1; index >= 0; index--) {
    if (isResumeBoundary(sinceSession[index])) {
      boundaryIndex = index
      break
    }
  }
...
function newestFailure(entries: readonly ConnectionLogEntry[]): ConnectionLogEntry | undefined {
  let newest: ConnectionLogEntry | undefined
  for (let index = entries.length - 1; index >= 0; index--) {
