...
describe("decodeAccountsSnapshot", () => {
  ...
  it("defaults missing runtime targets for older host-only snapshots", () => {
    ...
    expect(snapshot.rateLimits.claudeTarget).toEqual({
      runtime: "host",
      wslDistro: null,
    });
    expect(snapshot.rateLimits.codexTarget).toEqual({
      runtime: "host",
      wslDistro: null,
    });
  });
  ...
  it("validates display-only provider identities", () => {
    ...
    expect(decodeAccountsSnapshot(snapshot).rateLimits.gemini?.provider).toBe(
      "gemini"
    );

    setPath(snapshot, ["rateLimits", "gemini", "provider"], "codex");
    expect(() => decodeAccountsSnapshot(snapshot)).toThrow(
  ...
  it.each([
    ...
    [
      "window percentages",
      ["rateLimits", "codex", "session", "usedPercent"],
      101,
    ],
    [
      "credit counts",
      ["rateLimits", "codex", "rateLimitResetCredits", "availableCount"],
      -1,
    ],
    [
      "credit status",
    ...
    [
      "credit expiry",
      ["rateLimits", "codex", "rateLimitResetCredits", "nextExpiresAt"],
      "soon",
    ],
