...
describe("hasActiveProviderUsage", () => {
  ...
  it("is false for an unavailable/error provider with no window data (no creds)", () => {
    expect(hasActiveProviderUsage(makeLimits({ status: "unavailable" }))).toBe(
      false
    );
    expect(
      hasActiveProviderUsage(makeLimits({ status: "error", error: "nope" }))
    ).toBe(false);
  });
});
...
describe("getInactiveProviderUsage", () => {
  it("returns inactive usage using the runtime rateLimits payload shape", () => {
    ...
    expect(
      getInactiveProviderUsage(snapshot, "claude", "account-1")?.rateLimits
    ).toBe(limits);
  });
});
...
describe("getWindowResetLabel", () => {
  ...
  it("is null when there are no limits or the window has no reset timestamp", () => {
    ...
    expect(
      getWindowResetLabel(makeLimits({ status: "ok" }), "session", now)
    ).toBe(null);
    expect(
      getWindowResetLabel(
        makeLimits({ status: "ok", session: makeWindow(null) }),
        "session",
        now
      )
    ).toBe(null);
  ...
  it("formats minutes, hours+minutes, and days+hours like the desktop tooltip", () => {
    expect(
      getWindowResetLabel(
        makeLimits({ session: makeWindow(now + 47 * min) }),
        "session",
        now
      )
    ).toBe("Resets in 47m");
  ...
  it("formats exact hours and exact days without a zero remainder", () => {
    expect(
      getWindowResetLabel(
        makeLimits({ session: makeWindow(now + 2 * hour) }),
        "session",
        now
      )
    ).toBe("Resets in 2h");
  ...
  it('reports "Resets now" for a reset timestamp in the past', () => {
    expect(
      getWindowResetLabel(
        makeLimits({ session: makeWindow(now - min) }),
        "session",
        now
      )
    ).toBe("Resets now");
...
describe("getUsageBarState", () => {
  ...
  it("shows loading for a fetching provider without a window", () => {
    expect(
      getUsageBarState(makeLimits({ status: "fetching" }), "weekly")
    ).toEqual({
      usedPercent: null,
      unavailable: false,
      loading: true,
...
describe("getActiveProviderRateLimits", () => {
  it("reads a display-only provider (grok) via its descriptor field", () => {
    ...
    expect(
      getActiveProviderRateLimits(makeSnapshot({ grokLimits: grok }), "grok")
    ).toBe(grok);
  });
  ...
  it("maps opencode-go wire id to the opencodeGo snapshot field", () => {
    ...
    (snapshot.rateLimits as { opencodeGo?: ProviderRateLimits }).opencodeGo =
      openCode;
  ...
  it("reads Antigravity from its dedicated snapshot field", () => {
    ...
    expect(
      getActiveProviderRateLimits(
        makeSnapshot({ antigravityLimits: antigravity }),
        "antigravity"
      )
    ).toBe(antigravity);
  });
