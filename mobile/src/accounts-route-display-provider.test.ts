...
vi.mock("./components/codex-reset-credit", () => ({
  getActiveCodexAccountIdForRateLimitTarget: () => null,
  getCodexResetCreditSummary: () => null,
}));
...
describe("accounts route display-only providers", () => {
  beforeEach(() => {
    ...
    dependencies.loadHosts
      .mockReset()
      .mockResolvedValue([{ id: "host-1", name: "Desk" }]);
    dependencies.subscribe
      .mockReset()
      .mockImplementation((_method, _params, emit) => {
