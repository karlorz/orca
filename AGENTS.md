...
3b. **Fork desktop tags:** `v1.4.190-0`, `v1.4.190-1`, … from `fork-main`. Base = newest upstream desktop tag (release candidates count; drop `-rc.N`). Never a bare `v1.4.x` without suffix. Unsigned/ad-hoc macOS via `fork-desktop-voice-release.yml`. Never rewrite a published tag. Never put DMG/ZIP on a mobile tag. Fork desktop builds (version contains `fork.voice`) resolve update feeds against `karlorz/orca` prereleases.
