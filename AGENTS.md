...
2. **Keep fork mobile marketing version equal to the current published upstream mobile train.** Train = newest `stablyai/orca` release tag matching `mobile-android-vX.Y.Z` exactly, not `main` `app.json` (today: `0.0.47`; upstream prerelease flag does not exclude a train). Sync auto-resolves `mobile/app.json` to that train + `max(fork,upstream) versionCode`, then auto-cuts `mobile-android-v<train>-0` when the fork has no suffix on that base. Extra `-1`/`-2` stay attended: `node config/scripts/fork-next-mobile-tag.mjs --write`. Never rewind Play `versionCode`.
...
3b. **Fork desktop tags:** `v1.4.190-0`, `v1.4.190-1`, … from `fork-main`. Base = newest upstream desktop tag (release candidates count; drop `-rc.N`). Never a bare `v1.4.x` without suffix. Multi-platform desktop prereleases (self-signed macOS, Linux AppImage/deb/rpm, unsigned Windows NSIS) via `fork-desktop-voice-release.yml` and secrets `ORCA_FORK_CSC_LINK` / `ORCA_FORK_CSC_KEY_PASSWORD`. Never rewrite a published tag. Never put desktop files on a mobile tag. Fork desktop builds (`X.Y.Z-N`) resolve update feeds against `karlorz/orca` prereleases. Ad-hoc installs cannot auto-update onto the first self-signed cut.
