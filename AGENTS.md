...
0. **Branches.** Default and working branch is `fork-main`. GitHub `main` is a disposable mirror of `stablyai/orca` `main`, refreshed by `.github/workflows/fork-sync-main.yml` via `merge-upstream` (`branch=main` only; needs `FORK_SYNC_TOKEN` with `repo` + `workflow`). The same workflow then merges `stablyai/orca` `main` into `fork-main` with `git merge --no-ff` (`config/scripts/fork-sync-fork-main.mjs`), mirrors upstream `v*` tags into the fork (`config/scripts/fork-sync-upstream-tags.mjs`), auto-cuts `v<base>-0` on new upstream desktop bases (`config/scripts/fork-next-desktop-tag.mjs --auto`), and reports the next attended fork mobile tag (`config/scripts/fork-next-mobile-tag.mjs`, dry-run only). Daily 01:17 UTC (09:17 HKT) + `workflow_dispatch`. Never use GitHub **Sync fork** (it follows the default branch and can re-mirror deleted branches). Never call `merge-upstream` on `fork-main` (upstream has no such branch). Never commit product work on `main`. Record each landed fork feat in `config/fork-features.yml` (`node config/scripts/fork-features.mjs list`). The workflow reports **desktop** (`v1.4.x`, including `-rc.N`) and **mobile** (`mobile-android-vX.Y.Z`) latest upstream releases separately.
...
2. **Keep fork mobile marketing version equal to the current upstream mobile train base.** When trains report `mobile-android-v0.0.46`, the next attended fork tag is `mobile-android-v0.0.46-0` and `mobile/app.json` `expo.version` must become `0.0.46` with `versionCode` +1. Do not auto-cut mobile tags from sync. Detect with `node config/scripts/fork-next-mobile-tag.mjs`.
3. **Fork mobile tags:** `mobile-android-v<upstream-base>-N` (train `0.0.46` → `mobile-android-v0.0.46-0`). Never `fork-voice-v*`, never `-karlorz.N`.
3b. **Fork desktop tags:** `v1.4.190-0`, `v1.4.190-1`, … from `fork-main`. Base = newest upstream desktop tag (release candidates count; drop `-rc.N`). Never a bare `v1.4.x` without suffix. Unsigned/ad-hoc macOS via `fork-desktop-voice-release.yml`. Never rewrite a published tag. Never put DMG/ZIP on a mobile tag. Fork desktop builds (`X.Y.Z-N`) resolve update feeds against `karlorz/orca` prereleases.
