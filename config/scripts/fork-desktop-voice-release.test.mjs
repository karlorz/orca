...
describe('fork desktop voice release workflow', () => {
  ...
  it('fails closed without fork self-signed secrets and trusts that identity before electron-builder', () => {
    ...
    expect(trustSource).toContain('add-trusted-cert')
    expect(trustSource).toContain('trustRoot')
    expect(trustSource).toContain('find-identity')
    expect(trustSource).toContain('authorizationdb')
    expect(trustSource).toContain('/Library/Keychains/System.keychain')
    expect(trustSource).toContain('timeout=30')
