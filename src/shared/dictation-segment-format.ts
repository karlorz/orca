// CJK-aware spacing between committed dictation segments. Lifted from the
// renderer dictation module so the main-process mobile controller and the
// mobile app join segments with identical rules.
...
export function shouldInsertSpaceBetweenFinalSegments(
  previousText: string,
  nextText: string
): boolean {
...
// Join committed segments (and optionally a trailing partial) with the same
// spacing rules used when inserting segments one by one.
export function joinTranscriptSegments(segments: readonly string[]): string {
  ...
  for (const segment of segments) {
    ...
    joined += formatFinalTranscriptSegment(segment, joined)
  }
  return joined
}
