...
describe('OrcaRuntimeRpcServer pet.speak mobile allowlist', () => {
  it('allows pet.speak.accepted on a mobile-scoped token', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'orca-runtime-rpc-pet-accepted-'))
    ...
    await server['handleWebSocketMessage'](
      JSON.stringify({
        id: 'req_pet_speak_accepted',
        method: 'pet.speak.accepted',
        deviceToken: mobile.token,
        params: { event_id: 'ev-accepted-mobile' }
      }),
      (response) => replies.push(JSON.parse(response) as Record<string, unknown>),
      () => {}
    )
    expect(replies).toContainEqual(
      expect.objectContaining({ id: 'req_pet_speak_accepted', ok: true })
    )
  })
